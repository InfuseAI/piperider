...
class AssertionRecommender:
    ...
    def prepare_assertion_template(self, profiling_result):
        # It's a custom function to append yaml comment without space after '#'
        # by manipulating comment attribute in CommentedBase
        def _append_custom_yaml_comment_before_key(base, key, before, indent):
            c = base.ca.items.setdefault(key, [None, [], None, None])
            c[1].append(CommentToken(f'#{before}\n', CommentMark(indent)))
        ...
        for name, table in profiling_result.get('tables', {}).items():
            ...
            for col in table.get('columns', {}).keys():
                ...
                _append_custom_yaml_comment_before_key(columns[column_name],
                                                       key='tests', before="description: ''", indent=6)
            ...
            _append_custom_yaml_comment_before_key(recommended_assertion[name],
                                                   key='tests', before="description: ''", indent=2)
