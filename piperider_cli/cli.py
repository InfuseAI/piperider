...
@click.option('--no-auto-search', type=click.BOOL, default=False, is_flag=True, help="Disable searching for dbt projects.")
...
def init(**kwargs):
    'Initialize PipeRider configurations in interactive mode.'
...
@cli.command(short_help='Check configurations.')
@add_options(debug_option)
def diagnose(**kwargs):
    'Check config, datasource, connections, assertion config and dbt manifest.'
...
@cli.command(short_help='Profile, assert, and generate reports.')
@click.option('--datasource', default=None, type=click.STRING, help='Datasource to run.', metavar='DATASOURCE_NAME')
@click.option('--table', default=None, type=click.STRING, help='Table to run.', metavar='TABLE_NAME')
...
@add_options(debug_option)
def run(**kwargs):
    'Profile, assert, and generate reports.'
...
def compare_report(**kwargs):
    'Compare two existing reports in interactive mode or specified reports.'
